...
var uploadListIns = upload.render({
  elem: '#testList'
  ,url: "/Practice04/goodsPicController/upImage.do"
  ,data: {code:code}
  ,accept: 'images'
  ,multiple: true
  ,auto: false
...
init();
function init(){
ajax('/goodsPicController/getImage.do', {code:code}, 'json', function(data){
	console.log(data)
	var html = "" ;
    $.each(data,function(i, dom) {
        var id = dom.id;
        html+="<tr><td>"+dom.url+"</td><td>"
            +'<img src="/picture/'+ dom.url +'" alt="'+ dom.url +'" class="layui-upload-img">'
            +"</td><td>"
...
function delPic(id,code,url){
	openConfirm(function(index){
        ajax('/goodsPicController/del.do', {id:id,code:code,url:url}, 'text', function(data){
            if (data == 1) {
                layer.msg('删除成功');
                init();
            } else {
                layer.msg('删除失败，该编号已被使用');
            }
        })
    })
}
function setMain(id){
	openConfirm(function(index){
        ajax('/goodsPicController/setMain.do', {id:id,code:code}, 'json', function(data){
            if (data == 1) {
                layer.msg('设置成功');
                init();
            } 
