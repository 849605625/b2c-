...
<!-- start banner_x -->
			...
			<div class="jiesuandan mt20 center">
				<div class="tishi fl ml20">
					<ul>
						...
						<li>|</li>
						<li>共<span id="allnumber"></span>件商品
						<div class="clear"></div>
					</ul>
				</div>
   ...
   <div class="layui-form-item">
	...
	<script type="text/javascript">
	         $("#show2").hide();
             var user = "${user.code}"
             var canshu='<%=request.getParameter("ids")==null?"":request.getParameter("ids")%>'
             var all=0.0;
             var allnumber=0;
             var string="";
             var goodsCo=[];
             ...
             function init(){
            	 if(canshu==''){
                  ...
                  ajax('/carController/getlist.do', {canshu:canshu}, 'json', function(data){
                	$.each(data,function(i,dom){
                		goodsCo.push(dom.goodsCode)
                		goodsNu.push(dom.number)
                		var sum1=sum(dom.number,dom.price)
                		goodspri.push(sum1)
                   	var shu={name:dom.name,price:dom.price,number:dom.number,sum:sum1,pic:dom.pic,goodsCode:dom.goodsCode}
                   	var temp=$("#tradd").html();
		...
		function xiadan() {
			if (biao == 1) {
				ajax('/orderInforController/many.do', {
					userCode : user,
					orderAddr : $("select[name='region']").val(),
					goodspri : JSON.stringify(goodspri),
					goodsCo : JSON.stringify(goodsCo),
					goodsNu : JSON.stringify(goodsNu),
		...
		function mydingdan(){
			...
			<div class="content2 center">
                  ...
                  <input class="shuliang" id="{{d.goodsCode}}" value="{{d.number}}" readonly>
                   </div>
					</div>
